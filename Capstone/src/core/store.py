...
core/store.py
-------------
Cross-process shared store (SQLite).

WHY THIS EXISTS
===============
The FastAPI webhook (api/webhook.py) and the Streamlit dashboard (app.py) run
as TWO SEPARATE OS PROCESSES. A Python dict created in one process is invisible
to the other. That is exactly why alerts POSTed to the webhook never showed up
on the dashboard — the webhook stored them in its own memory.
